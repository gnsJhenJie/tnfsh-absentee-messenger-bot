...
def send_to_all_job():
    ...
    transfer_doc_ref = db.collection('transfer').document('send_to_third_grade')
    ...
    if format(transfer_docs_dic['status']) == 'pending': #發現未發送過的廣播訊息
        ...
        for recipient_id in to_send_list:
            user_info_doc_ref = user_info_collection_ref.document(recipient_id)
            docdict = user_info_doc_ref.get().to_dict()
